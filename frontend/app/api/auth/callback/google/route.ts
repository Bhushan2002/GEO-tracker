...
export async function GET(request: Request){
    ...
    if (!code) {
        return NextResponse.redirect(new URL('/google-analytics?error=no_code', request.url));
    }
    ...
    try{
        ...
        if (accounts.length === 0) {
            return NextResponse.redirect(new URL('/google-analytics?error=no_accounts', request.url));
        }
        ...
        if (properties.length === 0) {
            return NextResponse.redirect(new URL('/google-analytics?error=no_properties', request.url));
        }
        ...
        try {
            ...
            const aiTrafficAudienceExists = existingAudiences.some(
                (aud: any) => aud.displayName?.toLowerCase().includes("ai traffic")
            );
        ...
        const response = NextResponse.redirect(new URL('/google-analytics', request.url));
    ...
    return NextResponse.redirect(new URL('/google-analytics?error=setup_failed', request.url));
  }
}
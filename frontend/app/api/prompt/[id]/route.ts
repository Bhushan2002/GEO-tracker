...
export async function GET(
  ...
  try {
    ...
    console.log(`[DEBUG] Found ${allPromptRuns.length} runs for prompt ${promptId} in workspace ${workspaceId}`);
    ...
    console.log(`[DEBUG] Found ${promptRuns30d.length} runs in the last 30 days`);
    ...
    console.log(`[DEBUG] Found ${modelResponses.length} model responses total`);
    console.log(`[DEBUG] First response has brands: ${modelResponses[0]?.identifiedBrands?.length || 0}`);
